...
const Tourism = () => {
  ...
  useEffect(() => {
    tourisStore.getPaginatedTourismRequest(localStorage.getItem('user_etno_locality')!, searchFilter, pageNumber, 5)
  }, [pageNumber])
  const incrementPage = () => {
  ...
  function deleteByGroup() {
    if (tourisStore.getTourismCheckedList.length > 0) {
      tourisStore.deleteAllById(localStorage.getItem('user_etno_locality')!)
    }
  }
  return (
    <div className="w-full h-full  relative">
      <div className="flex flex-col gap-4">
        ...
        <TableTourism currentPage={pageNumber} headerList={['Seleccionar', 'tipo', 'Título', 'Descripción', 'Acciones']} />
      </div>
