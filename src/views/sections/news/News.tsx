  ...
  return (
    <div className="w-full h-full  relative">
      <div className="flex flex-col gap-4">
        <div className="flex flex-row">
          ...
          <div className="ml-auto">
            <button onClick={() => navigate("/addNews")} type="button" className="inline-flex items-center rounded-md border border-gray-300 bg-indigo-600 px-4 py-2 text-sm font-medium text-white    shadow-sm hover:bg-indigo-700 hover:shadow-md focus:ring-2 focus:ring-indigo-500">
        ...
        <TableNews currrentPage={pageNumber} headerList={['Categoría', 'Título', 'fecha', 'Descripción', 'Acciones']} />
      </div>
      <div className="flex absolute left-0 bottom-0 right-0  items-center justify-center md:flex-row flex-col">
        ...
        <button onClick={incrementPage} disabled={pageNumber == newsStore.getPaginatedNews.totalPages!! - 1 || newsStore.getPaginatedNews.content?.length == 0}  className="inline-flex items-center rounded-md border  disabled:bg-gray-500 border-gray-300 bg-indigo-800 px-4 py-3 text-sm font-medium text-gray-300 shadow-sm hover:bg-indigo-700 hover:shadow-md focus:ring-2 focus:ring-indigo-500">
          Siguiente
